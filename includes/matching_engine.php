...
/**
 * matching_engine.php
 * Système d'évaluation de pertinence (Local)
 * Compare les informations du candidat avec le concours ciblé.
 */

function calculate_relevance_score($candidat_profil, $offre_data) {
    ...
    if ($nombre_offre > 0) {
        $ratio = $nombre_communs / min(10, $nombre_offre);
        if ($ratio > 1) $ratio = 1;
        $score += $ratio * 70;
    }
    ...
    $score = min(max(round($score), 10), 98);
